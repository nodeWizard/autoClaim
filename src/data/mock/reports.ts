...
function buildEvidence(claim: (typeof initialClaims)[number]): Evidence[] {
  ...
  return claim.agentFindings.slice(0, 3).flatMap((af, i) =>
...
function recommendationsFor(claim: (typeof initialClaims)[number]): string[] {
...
const sourceClaims = initialClaims.filter(
  (c) => c.featured || c.agentFindings.length > 0 || (c.riskScore != null && c.riskScore >= 50)
)

export const reports: ClaimReport[] = sourceClaims.map((claim) => {
  const riskScore =
  ...
  return {
    ...
    generatedAt: claim.timeline.find((t) => t.title.toLowerCase().includes('rapport'))?.at
      ?? claim.declaredAt,
    ...
    aiReasoning: claim.agentFindings.flatMap((af) =>
...
export function getReport(id: string): ClaimReport | undefined {
  return reports.find((r) => r.id === id)
}

export function getReportByClaimId(claimId: string): ClaimReport | undefined {
